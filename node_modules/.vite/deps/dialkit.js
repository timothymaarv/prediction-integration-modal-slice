"use client";
import { i as __toESM, t as require_react } from "./react-BVvtt5Fv.js";
import { t as require_react_dom } from "./react-dom.js";
import { t as require_jsx_runtime } from "./react_jsx-runtime.js";
import { D as animate, Q as motion, V as useMotionValue, ht as AnimatePresence, z as useTransform } from "./react-GCo0l4Z-.js";
//#region node_modules/.pnpm/dialkit@1.2.0_motion@12.38.0_react-dom@19.2.5_react@19.2.5__react@19.2.5__react-dom@19.2.5_react@19.2.5__react@19.2.5/node_modules/dialkit/dist/index.js
...
var DialStoreClass = class {
	...
	updatePanel(id, name, config, shortcuts) {
		...
		const nextValues = {};
		for (const [path, defaultValue] of Object.entries(defaultValues)) nextValues[path] = this.normalizePreservedValue(existing.values[path], defaultValue, controlsByPath.get(path));
		...
		for (const [path, mode] of Object.entries(existing.values)) {
			...
			const transitionPath = path.slice(0, -7);
			if (controlsByPath.get(transitionPath)?.type === "transition") nextValues[path] = mode;
		}
		const nextPanel = {
			id,
			name,
		...
		for (const [path, defaultValue] of Object.entries(defaultValues)) nextBaseValues[path] = this.normalizePreservedValue(previousBaseValues[path], defaultValue, controlsByPath.get(path));
		for (const [path, value] of Object.entries(nextValues)) if (path.endsWith(".__mode")) nextBaseValues[path] = value;
	...
	subscribe(panelId, listener) {
		if (!this.listeners.has(panelId)) this.listeners.set(panelId, /* @__PURE__ */ new Set());
	...
	subscribeActions(panelId, listener) {
		if (!this.actionListeners.has(panelId)) this.actionListeners.set(panelId, /* @__PURE__ */ new Set());
	...
	resolveShortcutTarget(key, modifier) {
		for (const panel of this.panels.values()) for (const [path, shortcut] of Object.entries(panel.shortcuts)) {
			if (!shortcut.key) continue;
			if (shortcut.key.toLowerCase() !== key.toLowerCase()) continue;
			if ((shortcut.modifier ?? void 0) !== modifier) continue;
	...
	initTransitionModes(config, prefix, values) {
		for (const [key, value] of Object.entries(config)) {
			...
			if (this.isEasingConfig(value)) values[`${path}.__mode`] = "easing";
			...
			} else if (typeof value === "object" && value !== null && !Array.isArray(value) && !this.isActionConfig(value) && !this.isSelectConfig(value) && !this.isColorConfig(value) && !this.isTextConfig(value)) this.initTransitionModes(value, path, values);
		}
	}
	parseConfig(config, prefix, shortcuts) {
		...
		for (const [key, value] of Object.entries(config)) {
			...
			if (Array.isArray(value) && value.length <= 4 && typeof value[0] === "number") controls.push({
				type: "slider",
				path,
				label,
				min: value[1],
			...
			else if (this.isSpringConfig(value) || this.isEasingConfig(value)) controls.push({
				type: "transition",
				path,
				label
			});
			else if (this.isActionConfig(value)) controls.push({
				type: "action",
				path,
				label: value.label || label
			});
			else if (this.isSelectConfig(value)) controls.push({
				type: "select",
				path,
				label,
				options: value.options
			});
			else if (this.isColorConfig(value)) controls.push({
				type: "color",
				path,
				label
			});
			else if (this.isTextConfig(value)) controls.push({
				type: "text",
				path,
				label,
				placeholder: value.placeholder
			});
			else if (typeof value === "string") if (this.isHexColor(value)) controls.push({
				type: "color",
				path,
				label
			});
	...
	flattenValues(config, prefix) {
		...
		for (const [key, value] of Object.entries(config)) {
			if (key === "_collapsed") continue;
			const path = prefix ? `${prefix}.${key}` : key;
			if (Array.isArray(value) && value.length <= 4 && typeof value[0] === "number") values[path] = value[0];
			else if (typeof value === "number" || typeof value === "boolean" || typeof value === "string") values[path] = value;
			else if (this.isSpringConfig(value) || this.isEasingConfig(value)) values[path] = value;
			else if (this.isActionConfig(value)) values[path] = value;
			...
			} else if (this.isColorConfig(value)) values[path] = value.default ?? "#000000";
			else if (this.isTextConfig(value)) values[path] = value.default ?? "";
			else if (typeof value === "object" && value !== null) Object.assign(values, this.flattenValues(value, path));
		}
		return values;
	}
	...
	normalizePreservedValue(existingValue, defaultValue, control) {
		if (existingValue === void 0 || !control) return defaultValue;
		switch (control.type) {
			case "slider": {
				if (typeof existingValue !== "number" || typeof defaultValue !== "number") return defaultValue;
				const min = control.min ?? Number.NEGATIVE_INFINITY;
				const max = control.max ?? Number.POSITIVE_INFINITY;
				const clamped = Math.min(max, Math.max(min, existingValue));
				if (typeof control.step !== "number" || control.step <= 0) return clamped;
				return this.roundToStep(clamped, min, max, control.step);
			}
			case "toggle": return typeof existingValue === "boolean" ? existingValue : defaultValue;
			case "select": {
				if (typeof existingValue !== "string") return defaultValue;
				const options = control.options ?? [];
				return new Set(options.map((option) => typeof option === "string" ? option : option.value)).has(existingValue) ? existingValue : defaultValue;
			}
			case "color":
			case "text": return typeof existingValue === "string" ? existingValue : defaultValue;
			case "transition":
				if (this.isSpringConfig(defaultValue)) return this.isSpringConfig(existingValue) ? existingValue : defaultValue;
				if (this.isEasingConfig(defaultValue)) return this.isEasingConfig(existingValue) ? existingValue : defaultValue;
				return defaultValue;
			case "action": return defaultValue;
...
function useDialKit(name, config, options) {
	const panelId = `${name}-${(0, import_react.useId)()}`;
	...
	return buildResolvedValues(config, (0, import_react.useSyncExternalStore)((callback) => DialStore.subscribe(panelId, callback), () => DialStore.getValues(panelId), () => DialStore.getValues(panelId)), "");
}
function buildResolvedValues(config, flatValues, prefix) {
	...
	for (const [key, configValue] of Object.entries(config)) {
		if (key === "_collapsed") continue;
		const path = prefix ? `${prefix}.${key}` : key;
		if (Array.isArray(configValue) && configValue.length <= 4 && typeof configValue[0] === "number") result[key] = flatValues[path] ?? configValue[0];
		else if (typeof configValue === "number" || typeof configValue === "boolean" || typeof configValue === "string") result[key] = flatValues[path] ?? configValue;
		else if (isSpringConfig(configValue) || isEasingConfig(configValue)) result[key] = flatValues[path] ?? configValue;
		else if (isActionConfig(configValue)) result[key] = flatValues[path] ?? configValue;
		else if (isSelectConfig(configValue)) {
			const defaultValue = configValue.default ?? getFirstOptionValue(configValue.options);
			result[key] = flatValues[path] ?? defaultValue;
		} else if (isColorConfig(configValue)) result[key] = flatValues[path] ?? configValue.default ?? "#000000";
		else if (isTextConfig(configValue)) result[key] = flatValues[path] ?? configValue.default ?? "";
		else if (typeof configValue === "object" && configValue !== null) result[key] = buildResolvedValues(configValue, flatValues, path);
	}
	return result;
}
...
function formatSliderShortcut(sc) {
	const actionLabel = formatInteractionLabel(sc.interaction ?? "scroll");
	if (!sc.key) return actionLabel;
	return `${formatModifier(sc.modifier)}${sc.key.toUpperCase()}+${actionLabel}`;
}
function formatToggleShortcut(sc) {
	if (!sc.key) return "Press";
	return `${formatModifier(sc.modifier)}${sc.key.toUpperCase()}`;
}
function formatModifier(modifier) {
	return modifier === "alt" ? "⌥" : modifier === "shift" ? "⇧" : modifier === "meta" ? "⌘" : "";
}
var ShortcutContext = (0, import_react.createContext)({
	activePanelId: null,
	activePath: null
});
function ShortcutListener({ children }) {
	const [activeShortcut, setActiveShortcut] = (0, import_react.useState)({
		activePanelId: null,
		activePath: null
	});
	const activeKeysRef = (0, import_react.useRef)(/* @__PURE__ */ new Set());
	const isDraggingRef = (0, import_react.useRef)(false);
	const lastMouseXRef = (0, import_react.useRef)(null);
	const dragAccumulatorRef = (0, import_react.useRef)(0);
	const resolveActiveTarget = (0, import_react.useCallback)((interaction) => {
		for (const key of activeKeysRef.current) {
			const panels = DialStore.getPanels();
			for (const panel of panels) for (const [path, shortcut] of Object.entries(panel.shortcuts)) {
				if (!shortcut.key) continue;
	...
	(0, import_react.useEffect)(() => {
		...
		const handleKeyUp = (e) => {
			...
			lastMouseXRef.current = null;
			dragAccumulatorRef.current = 0;
			if (activeKeysRef.current.size === 0) setActiveShortcut({
				activePanelId: null,
				activePath: null
			});
			else {
		...
		const handleWheel = (e) => {
			if (isInputFocused()) return;
			const modifier = getActiveModifier(e);
			if (activeKeysRef.current.size > 0) for (const key of activeKeysRef.current) {
				const target = DialStore.resolveShortcutTarget(key, modifier);
				if (!target) continue;
				const { panelId, path, control } = target;
				if ((control.shortcut?.interaction ?? "scroll") !== "scroll" || control.type !== "slider") continue;
				e.preventDefault();
				applySliderDelta(panelId, path, control, getEffectiveStep(control, control.shortcut), e.deltaY > 0 ? -1 : 1);
				return;
			}
			...
			for (const { panelId, path, control, shortcut } of scrollOnlyTargets) {
				...
				e.preventDefault();
				applySliderDelta(panelId, path, control, getEffectiveStep(control, shortcut), e.deltaY > 0 ? -1 : 1);
				return;
			}
		};
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ShortcutContext.Provider, {
...
function Folder({ title, children, defaultOpen = true, isRoot = false, inline = false, onOpenChange, toolbar }) {
	...
	const [isCollapsed, setIsCollapsed] = (0, import_react.useState)(!defaultOpen);
	const contentRef = (0, import_react.useRef)(null);
	const [contentHeight, setContentHeight] = (0, import_react.useState)(void 0);
	const [windowHeight, setWindowHeight] = (0, import_react.useState)(typeof window !== "undefined" ? window.innerHeight : 800);
	(0, import_react.useEffect)(() => {
		if (!isRoot) return;
	...
	const folderContent = /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			...
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "dialkit-folder-header-top",
				children: [
					isRoot ? isOpen && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						className: "dialkit-folder-title-row",
						children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "dialkit-folder-title dialkit-folder-title-root",
							children: title
						})
					}) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						className: "dialkit-folder-title-row",
						children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "dialkit-folder-title",
							children: title
						})
					}),
					isRoot && !inline && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("svg", {
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
							...
							d: ICON_PANEL.path,
							fill: "currentColor"
						}), ICON_PANEL.circles.map((c, i) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("circle", {
							cx: c.cx,
							cy: c.cy,
							r: c.r,
							fill: "currentColor",
					...
					!isRoot && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(motion.svg, {
			...
			}), isRoot && toolbar && isOpen && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
			...
			children: isOpen && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(motion.div, {
				...
				initial: isRoot ? void 0 : {
					height: 0,
					opacity: 0
				},
				animate: isRoot ? void 0 : {
					height: "auto",
					opacity: 1
				},
				exit: isRoot ? void 0 : {
				...
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "dialkit-folder-inner",
					children
				})
			})
		})]
	...
	if (isRoot) {
		if (inline) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
			className: "dialkit-panel-inner dialkit-panel-inline",
			children: folderContent
		});
		const panelStyle = isOpen ? {
			width: 280,
			height: contentHeight !== void 0 ? Math.min(contentHeight + 10, windowHeight - 32) : "auto",
			borderRadius: 14,
		...
		return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(motion.div, {
			...
			whileTap: !isOpen ? { scale: .9 } : void 0,
			transition: {
				type: "spring",
				visualDuration: .15,
				bounce: .3
...
var MAX_STRETCH = 8;
function Slider({ label, value, onChange, min = 0, max = 1, step = .01, unit, shortcut, shortcutActive }) {
	const wrapperRef = (0, import_react.useRef)(null);
	const trackRef = (0, import_react.useRef)(null);
	const inputRef = (0, import_react.useRef)(null);
	const labelRef = (0, import_react.useRef)(null);
	const valueSpanRef = (0, import_react.useRef)(null);
	const [isInteracting, setIsInteracting] = (0, import_react.useState)(false);
	const [isDragging, setIsDragging] = (0, import_react.useState)(false);
	const [isHovered, setIsHovered] = (0, import_react.useState)(false);
	const [isValueHovered, setIsValueHovered] = (0, import_react.useState)(false);
	const [isValueEditable, setIsValueEditable] = (0, import_react.useState)(false);
	const [showInput, setShowInput] = (0, import_react.useState)(false);
	const [inputValue, setInputValue] = (0, import_react.useState)("");
	const hoverTimeoutRef = (0, import_react.useRef)(null);
	const pointerDownPos = (0, import_react.useRef)(null);
	const isClickRef = (0, import_react.useRef)(true);
	const animRef = (0, import_react.useRef)(null);
	const wrapperRectRef = (0, import_react.useRef)(null);
	...
	(0, import_react.useEffect)(() => {
		if (!isInteracting && !animRef.current) fillPercent.jump(percentage);
	}, [
		percentage,
		isInteracting,
		fillPercent
	]);
	const positionToValue = (0, import_react.useCallback)((clientX) => {
		...
		const rawValue = min + Math.max(0, Math.min(1, sceneX / nativeWidth)) * (max - min);
	...
	const percentFromValue = (0, import_react.useCallback)((v) => (v - min) / (max - min) * 100, [min, max]);
	const computeRubberStretch = (0, import_react.useCallback)((clientX, sign) => {
		const rect = wrapperRectRef.current;
		if (!rect) return 0;
	...
	const handlePointerMove = (0, import_react.useCallback)((e) => {
		...
		if (!isClickRef.current) {
			const rect = wrapperRectRef.current;
			if (rect) if (e.clientX < rect.left) rubberStretchPx.jump(computeRubberStretch(e.clientX, -1));
			else if (e.clientX > rect.right) rubberStretchPx.jump(computeRubberStretch(e.clientX, 1));
	...
	const handlePointerUp = (0, import_react.useCallback)((e) => {
		...
		if (isClickRef.current) {
			...
			const snappedValue = (max - min) / step <= 10 ? Math.max(min, Math.min(max, min + Math.round((rawValue - min) / step) * step)) : snapToDecile(rawValue, min, max);
		...
		if (rubberStretchPx.get() !== 0) animate(rubberStretchPx, 0, {
			type: "spring",
			visualDuration: .35,
			bounce: .15
		});
	...
	(0, import_react.useEffect)(() => {
		if (isValueHovered && !showInput && !isValueEditable) hoverTimeoutRef.current = setTimeout(() => {
	...
	if (trackWidth && trackWidth > 0) {
		if (labelRef.current) leftThreshold = (LABEL_CSS_LEFT + labelRef.current.offsetWidth + HANDLE_BUFFER) / trackWidth * 100;
		if (valueSpanRef.current) rightThreshold = (trackWidth - VALUE_CSS_RIGHT - valueSpanRef.current.offsetWidth - HANDLE_BUFFER) / trackWidth * 100;
	}
	const valueDodge = percentage < leftThreshold || percentage > rightThreshold;
	const handleOpacity = !isActive ? 0 : valueDodge ? .1 : isDragging ? .9 : .5;
	const discreteSteps = (max - min) / step;
	const hashMarks = discreteSteps <= 10 ? Array.from({ length: discreteSteps - 1 }, (_, i) => {
		return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
			className: "dialkit-slider-hashmark",
			style: { left: `${(i + 1) * step / (max - min) * 100}%` }
		}, i);
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
		ref: wrapperRef,
		className: "dialkit-slider-wrapper",
		children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(motion.div, {
			...
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "dialkit-slider-hashmarks",
					children: hashMarks
				}),
				...
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)(motion.div, {
					...
					animate: {
						...
						scaleX: isActive ? 1 : .25,
						scaleY: isActive && valueDodge ? .75 : 1
					},
					transition: {
						scaleX: {
							type: "spring",
...
function SegmentedControl({ options, value, onChange }) {
	...
	const [pillStyle, setPillStyle] = (0, import_react.useState)(null);
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		ref: containerRef,
		children: [pillStyle && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
...
function Toggle({ label, checked, onChange, shortcut, shortcutActive }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: "dialkit-labeled-control",
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
			className: "dialkit-labeled-control-label",
			children: [label, shortcut && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
				className: `dialkit-shortcut-pill${shortcutActive ? " dialkit-shortcut-pill-active" : ""}`,
...
function SpringVisualization({ spring, isSimpleMode }) {
	...
	let mass;
	if (isSimpleMode) {
		const visualDuration = spring.visualDuration ?? .3;
	...
	const pathData = points.map(([time, value], i) => {
		...
		const y = height - ((value - minValue) / (valueRange || 1) * height * .6 + height * .2);
		return `${i === 0 ? "M" : "L"} ${x} ${y}`;
	}).join(" ");
	const gridLines = [];
	for (let i = 1; i < 4; i++) {
		const x = width / 4 * i;
		const y = height / 4 * i;
		gridLines.push(/* @__PURE__ */ (0, import_jsx_runtime.jsx)("line", {
			x1: x,
			y1: 0,
			x2: x,
			y2: height,
			...
			strokeWidth: "1"
		}, `v-${i}`), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("line", {
			x1: 0,
			y1: y,
			x2: width,
			y2: y,
...
function SpringControl({ panelId, path, label, spring, onChange }) {
	const mode = (0, import_react.useSyncExternalStore)((cb) => DialStore.subscribe(panelId, cb), () => DialStore.getSpringMode(panelId, path), () => DialStore.getSpringMode(panelId, path));
	const isSimpleMode = mode === "simple";
	const cache = (0, import_react.useRef)({
		simple: spring.visualDuration !== void 0 ? spring : {
			type: "spring",
			visualDuration: .3,
			bounce: .2
		},
		advanced: spring.stiffness !== void 0 ? spring : {
			type: "spring",
			stiffness: 200,
			damping: 25,
			mass: 1
	...
	if (isSimpleMode) cache.current.simple = spring;
	else cache.current.advanced = spring;
	const handleModeChange = (newMode) => {
		DialStore.updateSpringMode(panelId, path, newMode);
		if (newMode === "simple") onChange(cache.current.simple);
		else onChange(cache.current.advanced);
	};
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Folder, {
		title: label,
		defaultOpen: true,
		children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			style: {
			...
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)(SpringVisualization, {
					spring,
					isSimpleMode
				}),
				...
				isSimpleMode ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Slider, {
					label: "Duration",
...
function TransitionControl({ panelId, path, label, value, onChange }) {
	const mode = (0, import_react.useSyncExternalStore)((cb) => DialStore.subscribe(panelId, cb), () => DialStore.getTransitionMode(panelId, path), () => DialStore.getTransitionMode(panelId, path));
	const isEasing = mode === "easing";
	const isSimpleSpring = mode === "simple";
	const cache = (0, import_react.useRef)({
		easing: value.type === "easing" ? value : {
			type: "easing",
			duration: .3,
			ease: [
				1,
		...
		simple: value.type === "spring" && value.visualDuration !== void 0 ? value : {
			type: "spring",
			visualDuration: .3,
			bounce: .2
		},
		advanced: value.type === "spring" && value.stiffness !== void 0 ? value : {
			type: "spring",
			stiffness: 200,
			damping: 25,
			mass: 1
	...
	if (isEasing && value.type === "easing") cache.current.easing = value;
	else if (isSimpleSpring && value.type === "spring") cache.current.simple = value;
	else if (mode === "advanced" && value.type === "spring") cache.current.advanced = value;
	...
	const handleModeChange = (newMode) => {
		...
		if (newMode === "easing") onChange(cache.current.easing);
		else if (newMode === "simple") onChange(cache.current.simple);
		else onChange(cache.current.advanced);
	};
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Folder, {
		...
		children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			...
			children: [
				isEasing ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(EasingVisualization, { easing }) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)(SpringVisualization, {
					spring,
					isSimpleMode: isSimpleSpring
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "dialkit-labeled-control",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
				...
				isEasing ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Slider, {
						label: "x1",
						value: easing.ease[0],
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)(EaseTextInput, {
						ease: easing.ease,
						onChange: (newEase) => onChange({
							...easing,
							ease: newEase
						})
					})
				] }) : isSimpleSpring ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Slider, {
					label: "Duration",
					value: spring.visualDuration ?? .3,
...
function TextControl({ label, value, onChange, placeholder }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: "dialkit-text-control",
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", {
			className: "dialkit-text-label",
			children: label
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
			type: "text",
			className: "dialkit-text-input",
...
function SelectControl({ label, value, options, onChange }) {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
			...
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
				className: "dialkit-select-label",
				children: label
			}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "dialkit-select-right",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
					className: "dialkit-select-value",
		...
		}), portalTarget && (0, import_react_dom.createPortal)(/* @__PURE__ */ (0, import_jsx_runtime.jsx)(AnimatePresence, { children: isOpen && pos && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(motion.div, {
			ref: dropdownRef,
			...
			children: normalized.map((option) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
...
function ColorControl({ label, value, onChange }) {
	...
	function handleTextSubmit() {
		setIsEditing(false);
		if (HEX_COLOR_REGEX.test(editValue)) onChange(editValue);
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: "dialkit-color-control",
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
			className: "dialkit-color-label",
			children: label
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: "dialkit-color-inputs",
			children: [
...
function PresetManager({ panelId, presets, activePresetId, onAdd }) {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: "dialkit-preset-manager",
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
			ref: triggerRef,
			...
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
				className: "dialkit-preset-label",
				children: activePreset ? activePreset.name : "Version 1"
			}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(motion.svg, {
				className: "dialkit-select-chevron",
				viewBox: "0 0 24 24",
				fill: "none",
		...
		}), (0, import_react_dom.createPortal)(/* @__PURE__ */ (0, import_jsx_runtime.jsx)(AnimatePresence, { children: isOpen && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(motion.div, {
			ref: dropdownRef,
			...
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				...
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
					className: "dialkit-preset-name",
					children: "Version 1"
				})
			}), presets.map((preset) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "dialkit-preset-item",
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
					...
					children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("svg", {
						...
						strokeLinecap: "round",
						strokeLinejoin: "round",
						children: ICON_TRASH.map((d, i) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", { d }, i))
					})
				})]
			}, preset.id))]
		}) }), document.body)]
...
function Panel({ panel, defaultOpen = true, inline = false }) {
	...
	const [isPanelOpen, setIsPanelOpen] = (0, import_react.useState)(defaultOpen);
	const shortcutCtx = (0, import_react.useContext)(ShortcutContext);
	Object.keys(panel.shortcuts).length;
	const values = (0, import_react.useSyncExternalStore)((cb) => DialStore.subscribe(panel.id, cb), () => DialStore.getValues(panel.id), () => DialStore.getValues(panel.id));
...
Apply these values as the new defaults in the useDialKit call.`;
	...
	const renderControl = (control) => {
		...
		switch (control.type) {
			case "slider": return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Slider, {
				label: control.label,
				value,
			...
			case "spring": return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(SpringControl, {
				panelId: panel.id,
				path: control.path,
				label: control.label,
				spring: value,
			...
			case "transition": return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TransitionControl, {
				panelId: panel.id,
				path: control.path,
				label: control.label,
				value,
			...
			case "folder": return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Folder, {
				title: control.label,
				defaultOpen: control.defaultOpen ?? true,
				children: control.children?.map(renderControl)
			}, control.path);
			case "text": return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TextControl, {
				label: control.label,
				value,
				onChange: (v) => DialStore.updateValue(panel.id, control.path, v),
				placeholder: control.placeholder
			}, control.path);
			case "select": return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(SelectControl, {
				label: control.label,
				value,
				options: control.options ?? [],
				onChange: (v) => DialStore.updateValue(panel.id, control.path, v)
			}, control.path);
			case "color": return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ColorControl, {
				label: control.label,
				value,
				onChange: (v) => DialStore.updateValue(panel.id, control.path, v)
			}, control.path);
			case "action": return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
	...
	const toolbar = /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
		/* @__PURE__ */ (0, import_jsx_runtime.jsx)(motion.button, {
			...
			children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("svg", {
				...
				strokeLinecap: "round",
				strokeLinejoin: "round",
				children: ICON_ADD_PRESET.map((d, i) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", { d }, i))
			})
		}),
		/* @__PURE__ */ (0, import_jsx_runtime.jsx)(PresetManager, {
			panelId: panel.id,
			presets,
			activePresetId,
		...
		/* @__PURE__ */ (0, import_jsx_runtime.jsx)(motion.button, {
			...
			children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
				...
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(AnimatePresence, {
					initial: false,
					mode: "wait",
					children: copied ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(motion.svg, {
						viewBox: "0 0 24 24",
						fill: "none",
						stroke: "currentColor",
						strokeWidth: "2",
						...
						children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", { d: ICON_CHECK })
					}, "check") : /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(motion.svg, {
						viewBox: "0 0 24 24",
						fill: "none",
						style: {
							position: "absolute",
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
		className: "dialkit-panel-wrapper",
...
var isDevDefault = typeof process !== "undefined" && "development" ? true : typeof import.meta !== "undefined" && import.meta.env?.MODE ? import.meta.env.MODE !== "production" : true;
function DialRoot({ position = "top-right", defaultOpen = true, mode = "popover", theme = "system", productionEnabled = isDevDefault }) {
	if (!productionEnabled) return null;
	const [panels, setPanels] = (0, import_react.useState)([]);
	const [mounted, setMounted] = (0, import_react.useState)(false);
	const inline = mode === "inline";
	const panelRef = (0, import_react.useRef)(null);
	const [dragOffset, setDragOffset] = (0, import_react.useState)(null);
	const [activePosition, setActivePosition] = (0, import_react.useState)(position);
	const lastDragOffset = (0, import_react.useRef)(null);
	...
	(0, import_react.useEffect)(() => {
		...
		return DialStore.subscribeGlobal(() => {
			setPanels(DialStore.getPanels());
		});
	}, []);
	(0, import_react.useEffect)(() => {
		...
		const observer = new MutationObserver(() => {
			...
			if (!(inner.getAttribute("data-collapsed") === "true")) {
				if (dragOffset) {
					...
					setActivePosition(dragOffset.x + 21 < window.innerWidth / 2 ? "top-left" : "top-right");
				} else setActivePosition(position);
				setDragOffset(null);
			} else if (lastDragOffset.current) setDragOffset(lastDragOffset.current);
		});
	...
	const handlePointerUp = (0, import_react.useCallback)((e) => {
		...
		if (didDragRef.current) {
			...
			if (inner) {
				...
				inner.addEventListener("click", blocker, {
					capture: true,
					once: true
				});
			}
	...
	const content = /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ShortcutListener, { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
		...
		children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
			...
			children: panels.map((panel) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Panel, {
				panel,
...
function ButtonGroup({ buttons }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
		className: "dialkit-button-group",
		children: buttons.map((button, index) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
			className: "dialkit-button",
...
function formatShortcutKey(sc) {
	if (!sc.key) return "—";
	return `${sc.modifier === "alt" ? "⌥" : sc.modifier === "shift" ? "⇧" : sc.modifier === "meta" ? "⌘" : ""}${sc.key.toUpperCase()}`;
}
function formatInteraction(sc) {
	switch (sc.interaction ?? "scroll") {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(motion.button, {
		...
		children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("svg", {
			...
			children: [
				...
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", { d: "M6 10H6.01" }),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", { d: "M10 10H10.01" }),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", { d: "M14 10H14.01" }),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", { d: "M18 10H18.01" }),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", { d: "M8 14H16" })
			]
		})
	}), (0, import_react_dom.createPortal)(/* @__PURE__ */ (0, import_jsx_runtime.jsx)(AnimatePresence, { children: isOpen && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(motion.div, {
		ref: dropdownRef,
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "dialkit-shortcuts-title",
				children: "Keyboard Shortcuts"
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "dialkit-shortcuts-list",
				children: rows.map((row) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "dialkit-shortcuts-row",
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "dialkit-shortcuts-row-key",
							children: formatShortcutKey(row.shortcut)
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "dialkit-shortcuts-row-label",
							children: row.label
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "dialkit-shortcuts-row-mode",
							children: formatInteraction(row.shortcut)
						})
					]
				}, row.path))
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "dialkit-shortcuts-hint",
				children: "See pill badges on controls for keys"
			})
		]
	}) }), document.body)] });
}
//#endregion
export { ButtonGroup, ColorControl, DialRoot, DialStore, EasingVisualization, Folder, PresetManager, SelectControl, ShortcutsMenu, Slider, SpringControl, SpringVisualization, TextControl, Toggle, TransitionControl, useDialKit };

//# sourceMappingURL=dialkit.js.map